...
const Music = (() => {
  ...
  const play = songName => {
    ...
    if (!song) { console.error(`Song Name "${songName}" not found.`); return; }
    if (!loaded[songName]) {
      loaded[songName] = zzfxM(...song);
    }
    myAudioNode = zzfxP(...loaded[songName]);
    //play the song again when it ends
    myAudioNode.addEventListener('ended', () => play(songName));
  };
  const stop = () => myAudioNode.stop();
  return {
    play,
    stop,
  }
